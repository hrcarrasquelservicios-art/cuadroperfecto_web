...
const meetingNumber = value => { const match = String(value ?? '').match(/\d+/); return match ? Number(match[0]) : null; };
const canonical = j => {
  ...
  snapshot.sha256 = hashAnalysis(analysis);
  return {id:j.id,slug:`historial-${j.slug || j.id}`,track:j.hipodromo,meeting_number:meetingNumber(j.reunion),date:isoDate(j.fecha),status:winners.length?'FINALIZADA':j.id.includes('13-09')?'PRELIMINAR':'ANÁLISIS CERRADO',unit_value:null,analysis_snapshot_id:snapshot.id,gate:analysis.gate,intelligence:analysis.intelligence,races,tickets:ticket?[ticket]:[],results:result,_snapshot:snapshot};
};
